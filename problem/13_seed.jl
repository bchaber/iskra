...
xs = 0m:Δh:Lx
ys = 0m:Δh:Ly
e  = create_kinetic_species("e-",   200_000,-1qe, 1.00me, 5e5);
iAr = create_kinetic_species("Ar+", 200_000,+1qe, 3.99mp, 5e5);
Ar = FluidSpecies("Ar", 1.0, 0qe, 3.99mp, nAr*ones(nx+1, ny+1), T)
se  = create_thermalized_beam(e,   [Δh Δh], [0. 0. 0.]; T=T, rate=1/Δt)
...
grid    = create_axial_grid(xs, ys)
solver  = create_poisson_solver(grid, ε0)
pusher  = create_axial_boris_pusher()
species = [e, iAr, Ar]
...
function ParticleInCell.after_push(part, grid)
  ParticleInCell.discard!(part, grid; dims=2)
end
...
function iteration(i, t, dt)
  cd("/tmp")
  new_iteration("13_seed", i, t, dt) do it
    save_records(it, "e-/")
    save_records(it, "Ar+/")
